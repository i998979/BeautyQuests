...
public class QuestsConfiguration {
	...
	private static List<ItemStack> header = null;
	private static List<ItemStack> footer = null;
	private static ItemStack prevpage = null;
	private static ItemStack nextpage = null;
	/*
	 * 
	 * MODIFIED MY i998979
	 * 
	...
	static void initConfiguration(FileConfiguration config) {
		 ...
		 * 
		 * */
		slots = config.getInt("barItem.slots");
		header = new ArrayList<ItemStack>();
		for (ItemStack item : (List<ItemStack>) config.getList("barItem.header")) {
			header.add(item);
		}
		footer = new ArrayList<ItemStack>();
		for (ItemStack item : (List<ItemStack>) config.getList("barItem.footer")) {
			footer.add(item);
		}
		prevpage = config.getItemStack("barItem.prevpage");
		nextpage = config.getItemStack("barItem.nextpage");
		/*
		 * 
		 * MODIFIED MY i998979
		 * 
	...
	public static List<ItemStack> getHeader() {
		return header;
	}
	public static List<ItemStack> getFooter() {
		return footer;
	}
